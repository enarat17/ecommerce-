    ...
    return (
        <>
            ...
            <div className='flex flex-col items-center'>
                <HeaderSection pageTitle={{en:'Business Services',ar:'خدمات أعمالي'}} pageImage={HTFetched && headerImages.businessServices} breadCrumbArr={{en:['services'],ar:['الخدمات']}} breadCrumbLinkArr={['/services']}/>
                ...
                <div className="w-full flex flex-wrap gap-8 items-center justify-center bg-transparent p-4">
                    {currentServices.map((service, id) => (
                        <ServiceCard key={id} service={service} serviceType='business-services' isRtl={isRTL} />
                    ))}
                </div>
                ...
                {filteredServices.length === 0 && (
                    <p className="text-center text-3xl text-unitedPrimary mt-8">
                        {isRTL ? "لا توجد خدمات أعمال مطابقة لبحثك." : "No business services match your search."}
                    </p>
                )}
            </div>
        </>
