...
export default function FinancialServicesPage() {
    ...
    const filteredServices = Array.isArray(services) 
        ? services.filter(service => 
            service.category === "Auditing Services" &&
    ...
    return (
        <>
            <NavigationWrapper />
            <HeaderSection pageTitle={{en:'Auditing Services',ar:'خدمات المراجعة'}} pageImage={HTFetched && headerImages.auditingServices} breadCrumbArr={{en:['services'],ar:['الخدمات']}} breadCrumbLinkArr={['/services']}/>
                ...
                <div className="w-full flex flex-wrap gap-8 items-center justify-center bg-transparent p-4">
                    {currentServices.map((service, id) => (
                        <ServiceCard key={id} service={service} serviceType='auditing-services' isRtl={isRTL} />
                    ))}
                </div>
                ...
                {filteredServices.length === 0 && (
                    <p className="text-center text-3xl text-unitedPrimary mt-8">
                        {isRTL ? "لا توجد خدمات مراجعة مطابقة لبحثك." : "No auditing services match your search."}
                    </p>
                )}
        </>
    );
