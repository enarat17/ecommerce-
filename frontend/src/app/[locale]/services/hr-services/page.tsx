...
export default function HRServicesPage() {
    ...
    return (
        <>
            <NavigationWrapper />
            <HeaderSection pageTitle={{en:'HR Services',ar:'الخدمات الإدارية'}} pageImage={HTFetched && headerImages.HrServices} breadCrumbArr={{en:['services'],ar:['الخدمات']}} breadCrumbLinkArr={['/services']}/>
                ...
                <div className="w-full flex flex-wrap gap-8 items-center justify-center bg-transparent p-4">
                    {currentServices.map((service, id) => (
                        <ServiceCard key={id} service={service} serviceType='hr-services' isRtl={isRTL} />
                    ))}
                </div>
                ...
                {filteredServices.length === 0 && (
                    <p className="text-center text-3xl text-unitedPrimary mt-8">
                        {isRTL ? "لا توجد خدمات أعمال إدارية لبحثك." : "No hr services match your search."}
                    </p>
                )}
        </>
    );
