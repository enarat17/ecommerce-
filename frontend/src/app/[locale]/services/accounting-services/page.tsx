...
export default function AccountingServicesPage() {
    ...
    const filteredServices = Array.isArray(services) 
        ? services.filter(service => 
            service.category === "Accounting Services" &&
    ...
    return (
        <>
            <NavigationWrapper />
            <HeaderSection pageTitle={{en:'Accounting Services',ar:'الخدمات المحاسبية'}} pageImage={HTFetched && headerImages.accountingServices} breadCrumbArr={{en:['services'],ar:['الخدمات']}} breadCrumbLinkArr={[]}/>
                ...
                <div className="w-full flex flex-wrap gap-8 items-center justify-center bg-transparent mb-8 min-h-[303px] p-4">
                    {currentServices.map((service, id) => (
                        <ServiceCard key={id} service={service} serviceType='accounting-services' isRtl={isRTL} />
                    ))}
                </div>
                ...
                {filteredServices.length === 0 && (
                    <p className="text-center text-3xl text-unitedPrimary my-8 min-h-[303px]">
                        {isRTL ? "لا توجد خدمات محاسبية مطابقة لبحثك." : "No accounting services match your search."}
                    </p>
                )}
        </>
    );
