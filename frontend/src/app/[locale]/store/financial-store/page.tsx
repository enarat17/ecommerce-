...
export default function ProductsPage() {
    ...
    const { data: products, isLoading, isError } = useQuery({
        ...
        queryFn: () => getFilteredProducts('category=Financial'),
        refetchOnWindowFocus: false,
        staleTime: 1000 * 60 * 60,
        gcTime: 1000 * 60 * 60 * 24,
    });

    const {data:headerImages,isFetched:HIFetched} = useQuery({
    ...
    return (
        <>
            <NavigationWrapper />
            <HeaderSection pageTitle={{en:'Financial Store',ar:'المتجر المالي'}} pageImage={HIFetched && headerImages.Financial_Store} breadCrumbArr={{en:['store'],ar:['المتجر']}} breadCrumbLinkArr={['store']}/>
                ...
                <div className="w-full flex flex-wrap gap-4 items-center justify-center bg-transparent p-8">
                    {currentProducts.map((product, id) => (
                        <ProductCard key={id} product={product} storeType='financial-store' isRtl={isRTL} />
                    ))}
                </div>
